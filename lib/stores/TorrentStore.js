...
export default new class TorrentStore extends EventEmitter {
  constructor() {
    ...
    chrome.runtime.onMessage.addListener(function (message) {
      if (TorrentActionCreators[message.action]) {
        TorrentActionCreators[message.action](message.data)
      }
    })
    let bg = chrome.extension.getBackgroundPage()
    Object.keys(bg.currentState).forEach((message) => {
      if (TorrentActionCreators[bg.currentState[message].action]) {
        TorrentActionCreators[bg.currentState[message].action](bg.currentState[message].data)
      }
    })
