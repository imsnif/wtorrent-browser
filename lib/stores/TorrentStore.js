...
export default new class TorrentStore extends EventEmitter {
  constructor() {
    ...
    Object.keys(bg.torrentCache).forEach((message) => {
      if (TorrentActionCreators[bg.torrentCache[message].action]) {
        TorrentActionCreators[bg.torrentCache[message].action](bg.torrentCache[message].data)
      }
    })
