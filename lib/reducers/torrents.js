...
const torrent = (state, action) => {
  ...
  switch (action.type) {
    case 'ADD_TORRENT':
      let addTemp = Object.assign({status: "pending"}, torrentMetadata(action.magnetUri))
      console.log("addTemp status:", addTemp.status)
    ...
    case 'UPDATE_TORRENT':
      ...
      let updatedTemp = Object.assign({status}, torrentMetadata(action.data))
      console.log("updated:", updatedTemp.status)
